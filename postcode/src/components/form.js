...
*/
    deleteFromList(e) {
        ...
        let listElement = document.getElementById("selectedPLaceses-" + Number(index));
        console.log("selectedPLaceses-");
        listElement.classList.remove(`animated`);
        listElement.classList.remove(`${ Number(index)%2!==0?"rotateInUpLeft":"rotateInUpRight"}`);

        setTimeout(() => {
            listElement.classList.add(`animated`);
            listElement.classList.add('zoomOutLeft');
    ...
    pushNewPlace(data) {
        ...
        console.log(" selectedPLaceses BEFOR SENDING DATA : " + JSON.stringify(selectedPLaceses));
    ...
    handelSendDataToserver(e) {
        ...
        let data = {
            ...
            index = 0,
            length = data.selectedPLaceses.length,
            interval = setInterval(() => {
                let listElement = document.getElementById("selectedPLaceses-" + index);
                if (index !== length) {
                    listElement.classList.remove(`animated`);
                    listElement.classList.remove(`${index%2!==0?"rotateInUpLeft":"rotateInUpRight"}`);
                    listElement.classList.add(`animated`);
                    listElement.classList.add('zoomOutLeft');
                }
                else {

                    console.log("selectedPLaceses-" + index);
        ...
        setTimeout(() => {
            ...
            return axios.post(`${apiPrefix}/getNewUserData`, data).then((response) => {
                ...
                console.log(" SENDING TO DB ====  " + JSON.stringify(this.state.selectedPLaceses));
                this.setState({ selectedPLaceses: [] });
                console.log(" getting data from DB : " + JSON.stringify(response.data));
  ...
  <Row className="mt-5 mb-3 d-flex justify-content-center" >
   ...
   <Col xs={12} className=" text-left ">
  <ListGroup>
{this.state.selectedPLaceses.length!==0?this.state.selectedPLaceses.map((item,i)=>{ return <ListGroupItem
className={`animated ${ i%2!==0?"rotateInUpLeft":"rotateInUpRight"}`} 
key={"selectedPLaceses-"+i} id={"selectedPLaceses-"+i}> {item.adress} <button className=" ml-auto btn btn-danger btn-sm" onClick={this.deleteFromList} value={i} > delete </button> </ListGroupItem> }):""}
 </ListGroup>
  </Col>
   </Row>
